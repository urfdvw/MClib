classdef PMCConv < handle
    ...
    properties
        ...
        sig_conv % width of the convoluted Gaussian
    ...
    methods
        function O = PMCConv(logTarget, mu0, K)
        ...
        function sample(O)
            ...
            parfor n = 1:O.N
                ...
                logCw_n = zeros([O.K,1]); % log tempered weights of samples for current population
                for k = 1:O.K
                    ...
                    if O.sig_conv == 0
                        ...
                        logCw_n(k) = target_conv(@O.logTarget,O.sig_conv,x_n(k,:)) - logprop;
                    end
                end
                % record samples and unnormalized weights
                data_ip1{n} = Ppdf(x_n, logw_n);
                data_temp{n} = Ppdf(x_n, logCw_n);
            end
